    ...
    const fetchAssessment = async () => {
        try {
            ...
            if (response.ok) {
                ...
                setAssessorRole(a.assessorRole);
    ...
    const updateQuestion = (index: number, field: keyof Question, value: unknown) => {
        ...
        if (field === 'type') {
            ...
            if (value === 'multiple-choice' || value === 'checkbox') {
                // Preserve options if switching between choice types, else init simple defaults if empty
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        for (const q of questions) {
            ...
            if ((q.type === 'multiple-choice' || q.type === 'checkbox') && (!q.options || q.options.length < 2)) {
                setError('Multiple choice/checkbox questions must have at least 2 options');
                return;
            }
        }
    ...
    if (loading) {
        return (
            <main className="dashboard">
                <div className="dashboard__card">Loading editor...</div>
            </main>
        );
    }
        ...
        <main className="dashboard">
            <section className="dashboard__hero">
                <div>
                    ...
                    <p className="dashboard__muted">Modify assessment content and settings</p>
                </div>
            </section>
