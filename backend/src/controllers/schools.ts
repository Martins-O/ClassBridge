...
export async function getSchools(req: Request, res: Response) {
  try {
    ...
    const { filters, search } = getQueryParams(req, {
      allowedFilterFields: ['isActive', 'name', 'status'],
    ...
    if (isSystemAdmin(user.role as UserRole)) {
      result = await schoolService.getAll(userId, user.role, page, limit, query);
    } else {
      result = await schoolService.getApprovedSchools(page, limit, query);
    }
...
export async function createSchool(req: Request, res: Response) {
  try {
    ...
    try {
      const school = await schoolService.createDirect({
        name,
        email,
        phone,
        address,
...
export async function getSchoolById(req: Request, res: Response) {
  try {
    ...
    const canAccess = await schoolService.canAccess(userId, user.role, id);
    if (!canAccess) {
...
export async function getSchoolStatus(req: Request, res: Response) {
  try {
    await connectDB();
    ...
    const status = await schoolService.getStatus(user.schoolId.toString());
