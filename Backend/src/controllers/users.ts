...
export async function updateUser(req: Request, res: Response) {
  try {
    ...
    if (payload.role !== undefined) {
      validation.errors.push(
        ...validateEnum(payload.role, 'role', USER_ROLES.map((role: any) => role.value)),
      );
    }

    if (payload.schoolId !== undefined) {
      validation.errors.push(...validateObjectId(payload.schoolId, 'schoolId'));
    }
    ...
    if (payload.classIds !== undefined) {
      if (!Array.isArray(payload.classIds)) {
        ...
        validation.errors.push(
          ...validateArray(payload.classIds, 'classIds', { itemType: 'string' }),
        );
      }
    }
